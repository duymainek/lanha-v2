...
import type { Room, SupabaseTenant, InvoiceItem } from "@/data/types"
...
export function EditInvoiceDialog({
  ...
  const [form, setForm] = React.useState<InvoiceForm>(initialInvoice)
  ...
  // Khi chọn phòng, tự động cập nhật tenant và items nếu muốn cho phép đổi phòng khi edit
  const handleApartmentChange = async (apartmentId: number) => {
    ...
    setLoadingRoom(true);
    try {
      // Không fetch utility readings khi edit, chỉ đổi tenant nếu cần
      setForm(f => ({
        ...f,
  ...
  const handleItemChange = (idx: number, key: keyof InvoiceItem, value: string | number) => {
    setForm(f => {
      ...
      const total = subtotal + (f.additional_fees || 0) - (f.discounts || 0);
  ...
  const handleSubmit = () => {
    ...
    if (errors.length > 0) {
      toast.error('Please fix the errors below:', {
        description: <ul className="list-disc pl-5">{errors.map((err, idx) => <li key={idx}>{err}</li>)}</ul>
      });
