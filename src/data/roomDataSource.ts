...
import type { Room, SupabaseBuilding, SupabaseInvoiceRaw, UtilityReading, SupabaseTenant, Tenant, BuildingExpense, NotificationQueueItem } from './types';
 ...
 */
export async function updateBuildingInSupabase(id: string, data: { name: string; address: string; notes?: string }): Promise<SupabaseBuilding> {
...
export async function updateRoomInSupabase(id: number, data: Partial<Room>): Promise<Room> {
  // Map Room fields to Supabase apartments fields
  const updatePayload: any = {
    ...
    // type không lưu trực tiếp, có thể map sang bedrooms nếu cần
  };
    ...
    .select(`
     ...
     invoice_items!fk_invoice_items_invoice_id (id, invoice_id, item_type, description, quantity, unit_price, total, previous_reading, current_reading, discount, created_at),
      ...
      apartments:apartment_id (
        id, unit_number, building_id, area, price, status,
...
/**
 * Lấy danh sách chi phí tòa nhà, join với bảng buildings để lấy tên tòa nhà
