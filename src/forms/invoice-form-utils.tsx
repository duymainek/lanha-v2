    ...
    <div className="relative w-full" id="invoice-detail-print-area">
      <div className="flex-1 overflow-y-auto py-2 hide-scrollbar">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-2 md:gap-4 md:col-span-2 mt-2">
        ...
        <div className="mb-8">
          <div className="mb-4">
            ...
            <table className="min-w-full border border-border-color rounded-lg bg-white hidden md:table">
            ...
            <div className="md:hidden flex flex-col gap-3">
              {invoice.invoice_items.filter(i => i.item_type === 'rent').map((item, idx) => (
                <div key={idx} className="border rounded-lg bg-white p-3 shadow-sm">
                  <div className="flex justify-between text-xs text-muted-foreground mb-1"><span>Type</span><span>{item.item_type}</span></div>
                  <div className="flex justify-between text-xs mb-1"><span>Description</span><span>{item.description}</span></div>
                  <div className="flex justify-between text-xs mb-1"><span>Quantity</span><span>{item.quantity}</span></div>
                  <div className="flex justify-between text-xs mb-1"><span>Unit Price</span><span>{formatToVND(item.unit_price)}</span></div>
                  <div className="flex justify-between text-xs font-semibold"><span>Total</span><span>{formatToVND(item.total)}</span></div>
                </div>
              ))}
            </div>
          </div>
          <div className="mb-4">
            ...
            <div className="md:hidden flex flex-col gap-3">
              {invoice.invoice_items.filter(item => item.item_type !== 'rent').map((item, idx) => (
                <div key={idx} className="border rounded-lg bg-white p-3 shadow-sm">
                  <div className="flex justify-between text-xs text-muted-foreground mb-1"><span>Type</span><span>{item.item_type}</span></div>
                  <div className="flex justify-between text-xs mb-1"><span>Description</span><span>{item.description}</span></div>
                  {item.item_type === 'electricity' && (
                    <>
                      <div className="flex justify-between text-xs mb-1"><span>Prev</span><span>{item.previous_reading ?? ''}</span></div>
                      <div className="flex justify-between text-xs mb-1"><span>Current</span><span>{item.current_reading ?? ''}</span></div>
                    </>
                  )}
                  <div className="flex justify-between text-xs mb-1"><span>Quantity</span><span>{item.quantity}</span></div>
                  <div className="flex justify-between text-xs mb-1"><span>Unit Price</span><span>{formatToVND(item.unit_price)}</span></div>
