...
import { IconArrowLeft, IconEye, IconCode, IconDeviceFloppy, IconPlus, IconTrash, IconEdit, IconWand } from "@tabler/icons-react"
import type { ContractTemplateField, SupabaseContractTemplate } from "@/data/types"
import { ContractTemplateService } from "@/services/ContractTemplateService"
import { ContractService } from "@/services/ContractService"
import { 
  fetchContractTemplateByIdFromSupabase,
...
export default function EditContractTemplatePage() {
  ...
  const [originalTemplate, setOriginalTemplate] = useState<SupabaseContractTemplate | null>(null)
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      if (!id) {
        toast("Template ID is required")
        navigate('/contract-templates')
        return
      }

      try {
        const template = await fetchContractTemplateByIdFromSupabase(id)
        if (!template) {
  ...
  const handleSave = async () => {
    ...
    if (!originalTemplate) {
      toast("Original template not found")
      return
    }
    ...
    const validation = ContractTemplateService.validateTemplate({
      ...
      updated_at: originalTemplate.updated_at,
      file_url: originalTemplate.file_url,
    ...
    setIsSaving(true)
    try {
      await updateContractTemplateInSupabase(originalTemplate.id, {
    ...
    const generatePreview = async () => {
      ...
      try {
        const filledContent = await ContractService.fillTemplate({
          ...
          created_at: originalTemplate?.created_at || "",
          updated_at: originalTemplate?.updated_at || "",
          file_url: originalTemplate?.file_url || null,
          is_active: originalTemplate?.is_active || true
        }, sampleData)
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Modify template fields and content
                  </p>
                </div>
              </div>
